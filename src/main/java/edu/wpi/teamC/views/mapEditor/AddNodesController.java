...
public class AddNodesController {
    ...
    public void initialize(){
        initGetCoords();
        initialImageWidth=5000;
        initialImageHeight=3400;
        s.setPrevTime();
        ...
        AnchorPane.setBottomAnchor(hbox2, anchor.getPrefHeight()*422/680);
        ...
        AnchorPane.setBottomAnchor(hbox3, anchor.getPrefHeight()*322/680);

        hbox4.prefHeightProperty().bind(anchor.heightProperty().multiply(100).divide(680));
        AnchorPane.setBottomAnchor(hbox4, anchor.getPrefHeight()*222/680);

        hbox6.prefHeightProperty().bind(anchor.heightProperty().multiply(100).divide(680));
        AnchorPane.setBottomAnchor(hbox6, anchor.getPrefHeight()*222/680);

        hbox5.prefHeightProperty().bind(anchor.heightProperty().multiply(100).divide(680));
        //AnchorPane.setBottomAnchor(hbox5, anchor.getPrefHeight()*101/680);
    ...
    public void goEmergency(ActionEvent actionEvent) {
        try {
            Capp.prevPage = "/edu/wpi/teamC/fxml/mapEditor/AddNode.fxml";
    ...
    private void validateSubmit(){
        if(NodeID.getText().isEmpty() || xCoord.getText().isEmpty() || yCoord.getText().isEmpty() || floor.getText().isEmpty() || building.getText().isEmpty() || nodeType.getText().isEmpty() || longName.getText().isEmpty() || shortName.getText().isEmpty()) {
    ...
    public void submit(ActionEvent actionEvent) {
        ...
        String t = nodeType.getText();
        String l = longName.getText();
        String s = shortName.getText();

            NodeDB node = new NodeDB(id,x,y,f,b,t,l ,s);
    ...
    public void initGetCoords(){
        imgView.setOnMouseClicked(new EventHandler<MouseEvent>() {
            @Override
            public void handle(MouseEvent event) {

                xCoord.setText(Integer.toString(transformX(event.getSceneX())));
                yCoord.setText(Integer.toString(transformY(event.getSceneY())));
            }
        });
    }
    public int transformX(double location){
        return transform(location,initialImageWidth, imgView.getLayoutX(), imgView.getLayoutBounds().getWidth());
    }
    public int transformY(double location){
        return transform(location,initialImageHeight,imgView.getLayoutY(), imgView.getLayoutBounds().getHeight());
    }
