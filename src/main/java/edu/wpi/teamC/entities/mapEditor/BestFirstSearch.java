...
public class BestFirstSearch extends AbsSearch{
            ...
            for (Edge edge : edges) {
                ...
                if ((!costSoFar.containsKey(next)) || (newCost < costSoFar.get(next))) {//if cost for this node is currently
                    ...
                    if((emergencyRoute&&isEmergency)||(noEmergencyRoute&&!isEmergency)||(!noEmergencyRoute&&!emergencyRoute)) {
                        ...
                        double priority = NavAlgs.projectCost(goal, next); //priority considers estimated cost to get to goal(calculated with distance
